...
x, y = historical_shares_prices_func(IMOEX_figi, datetime.datetime(1999, 11, 4, tzinfo=datetime.timezone.utc), now(), CandleInterval.CANDLE_INTERVAL_MONTH)
...
plt.plot(np.array(x)[peaks], rolling_mean.iloc[peaks], 'ro', label='Пики точёные')
plt.plot(np.array(x)[extrema_min], rolling_mean.iloc[extrema_min], 'go', label='Хуи дрочённые')
